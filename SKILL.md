...
- 计划贡献评价（本周）：仅对计划名可识别为 US猫、US狗、CN猫、CN狗的计划计算 `推广收入×(0.5-0)×(新客率×新客目标权重+1)-2×花费`；推广收入为总成交金额，权重依次为 1.5、1.8、1.8、2。名称含“美国”为 US，否则 CN；含“猫”优先，否则含“狗”或“犬”为狗；其他计划不参与计算。计划矩阵下必须自动输出指标最高 3 个计划为“业务亮点”，以及所有指标<0计划为“需要调整”，并展示计算所用字段和值。
