...
| 计划—商品映射 | `references/plan-product-map.json`（可用环境变量 `HALO_PLAN_PRODUCT_MAP` 覆盖） | 是 |
...
2. **确定性计算**：运行 `scripts/analyze.py`，产出 `scripts/analysis.json`（artifact）。检查其 `meta.warnings`；阅读 stdout JSON 摘要。脚本输出 `plan_matrix`（同一计划包含 w0/w1 双周指标，横轴 ROI 中线 5、纵轴新客率中线 50%）、`plan_value`（本周计划贡献评价：业务亮点 Top 3 与指标<0计划）、`prod_sku`（全部 SKU 的双周推广费比，费比=花费÷总成交金额）与 `optimize`（ROI<5 优化清单：本周花费词 ≥500 元 / 人群、计划 ≥1,000 元）。
...
- 计划矩阵明细表增加 ROI变化率、新客率变化率（均按本周相对上周的百分比变化），所有字段可通过表头小三角升降序排序；本周花费数值旁展示花费环比涨跌箭头。矩阵上方必须提供“仅看 ROI、新客率均下降计划”按钮：开启后仅保留两项均严格低于上周的计划，并在点位及计划名旁标注两项下降百分比；上周视图的花费趋势显示“—”。
- 计划贡献评价（本周）：仅按 `references/plan-product-map.json` 中的计划—商品映射计算，禁止从计划名称识别。逐个商品名称识别 US猫、US狗、CN猫、CN狗：含“美国”为 US，否则 CN；含“猫”优先，否则含“狗”或“犬”为狗；权重依次为 1.5、1.8、1.8、2。同计划多商品按商品数等额分摊计划级推广收入和花费，等价于使用商品权重算术平均；禁止把计划总收入重复套给每个商品。公式为 `推广收入×(0.5-0)×(新客率×综合权重+1)-2×花费`。计划矩阵下必须自动输出指标最高 3 个计划为“业务亮点”，以及所有指标<0计划为“需要调整”，并展示商品组合识别、商品数、综合权重及计算字段。
- 计划矩阵的计划明细表不得展示“调整方向”字段；优化清单表不得展示“评价与动作”字段。两类表仅保留对应的客观指标列。
...
- 品销宝、营销场景、计划矩阵、关键词、人群、商品主体、优化清单的下方结论仅展示客观数据与环比/阈值判断，不展示主观指导、建议或动作；品销宝删除“承接”条目。
- 商品 SKU 费比图隐藏本周与上周费比均为 0 或不可计算的记录；优化清单隐藏上周花费为 0 的“新增”记录。
- 计划矩阵中，当本周较上周 ROI 下降超过 1 且新客率下降超过 20 个百分点时，点位显示红色边框；ROI 上升超过 1 且新客率上升超过 20 个百分点时，显示蓝色边框。
...
| 计划—商品映射缺失/非法 | 中断，提示补齐 `references/plan-product-map.json` 或设置 HALO_PLAN_PRODUCT_MAP（exit 2） |
